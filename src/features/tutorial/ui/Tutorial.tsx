...
const steps = [
  {
    title: "환영합니다! 👋",
    content:
      "Tickle은 당신의 AI 자산관리 파트너입니다. 주요 기능을 빠르게 살펴볼까요?",
    target: "body",
    position: "center",
  },
  {
...
export function Tutorial() {
  ...
  // Open sidebar from step 3 onwards (index >= 2)
  useEffect(() => {
  ...
  }, [currentStep, isOpen, isSidebarOpen, toggleSidebar]);
  ...
  if (isSpotlight && targetRect) {
    ...
    if (preferredPosition !== "center") {
      ...
      if (!fits) {
        ...
        if (
          targetRect.height > viewportHeight * 0.8 ||
          targetRect.width > viewportWidth * 0.8
        ) {
          altPos = "center";
        }
      ...
      <motion.div
        ...
        style={{
          ...
          transform: !isSpotlight
            ? "translate(-50%, -50%)"
            : `translate(${tooltipStyle.x}, ${tooltipStyle.y})`,
        }}
      >
        <button
          onClick={handleClose}
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between gap-2 sm:gap-3 pt-4">
            <button
              ...
              className="p-2 sm:p-2.5 rounded-lg hover:bg-muted disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              title="이전"
            >
              <ChevronLeft size={20} />
            </button>
            ...
            <button
              onClick={handleNext}
              className="flex items-center justify-center gap-1 sm:gap-2 px-3 sm:px-4 py-2.5 sm:py-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 active:scale-95 transition-all font-medium text-sm sm:text-base whitespace-nowrap"
            >

...
export function TransactionList() {
  ...
  if (!transactions?.length) {
    return <div className="p-4 text-center text-muted-foreground">거래 내역이 없습니다.</div>
  }
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{new Date(t.date).toLocaleDateString('ko-KR')}</p>
              </div>
            </div>
            <span className="font-medium text-destructive">-₩{t.amount.toLocaleString()}</span>
          </motion.div>
        )
      })}
    </div>
